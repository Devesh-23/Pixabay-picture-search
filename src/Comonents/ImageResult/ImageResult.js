...
function ImageResult() {
  ...
  useEffect(() => {
    axios.get(`${apiUrl}/?key=${apiKey}&q=${location.state.squery}&image_type=photos&safesearch=true`).then(
      (response) => {
  ...
  return (
    <div>
       <ImageList variant="masonry" cols={4} gap={6} className={styles.gridlist}>
  {pics.map((item) => (
